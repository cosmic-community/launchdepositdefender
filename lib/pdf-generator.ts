...
export class PDFGenerator {
  ...
  private addSummaryStats(property: Property): void {
    ...
    stats.forEach((stat, index) => {
      ...
      // Value - Fix: Provide proper null check for getTextWidth and ensure centerX is always a number
      this.doc.setFontSize(16)
      this.doc.setTextColor(37, 99, 235)
      const textWidth = this.doc.getTextWidth(value) || 0  // Fix: Use || 0 instead of ?? 0 for better compatibility
      const centerX = x + boxWidth / 2 - textWidth / 2
      this.doc.text(value, centerX, this.currentY + 12)
      
      // Label - Fix: Provide proper null check for getTextWidth and ensure labelCenterX is always a number
      this.doc.setFontSize(8)
      this.doc.setTextColor(100, 100, 100)
      const labelWidth = this.doc.getTextWidth(labelText) || 0  // Fix: Use || 0 instead of ?? 0 for better compatibility
  ...
  private centeredText(text: string, y: number): void {
    const textWidth = this.doc.getTextWidth(text) || 0  // Fix: Use || 0 instead of ?? 0 for better compatibility
  ...
  private addFooterToAllPages(property: Property): void {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      const addressWidth = this.doc.getTextWidth(address) || 0  // Fix: Use || 0 instead of ?? 0 for better compatibility
      const addressCenterX = this.pageWidth / 2 - addressWidth / 2
      this.doc.text(address, addressCenterX, this.pageHeight - 8)
      
      const pageText = `Page ${i} of ${totalPages}`
      const pageTextWidth = this.doc.getTextWidth(pageText) || 0  // Fix: Use || 0 instead of ?? 0 for better compatibility
